...
class SnowPointsShaderUtils  {

  @Language("agsl")
  private val shaderString =  """
...
"""
  ...
  private val runtimeShader = RuntimeShader(shaderString).apply {
    setIntUniform("uLayers", 15)
    setFloatUniform("uDepth", 0.2f)
    setFloatUniform("uSpeed", 0.8f)
    setFloatUniform("resolution", floatArrayOf(
      App.instance.getScreenWidth().toFloat(),
      App.instance.getScreenHeight().toFloat()
    ))
  }

  fun getRenderEffect() = RenderEffect.createRuntimeShaderEffect(runtimeShader, "image")
