...
class RefreshViewLayout(context: Context) : AViewGroup(context) {
    ...
    val loadingImageView = AppCompatImageView(context).apply {
        setImageResource(R.drawable.ic_loading_sun)
        layoutParams = LayoutParams(60.dp, 60.dp).apply {
            topMargin = 12.dp
            bottomMargin = 8.dp
        }
    }

    val tipText = AppCompatTextView(context).apply {
        setTextColor(Color.WHITE)
        setTextAppearance(R.style.TextAppearance_Material3_LabelMedium)
        layoutParams = LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        )
